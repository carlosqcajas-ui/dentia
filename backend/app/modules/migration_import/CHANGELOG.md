...
- fix(catalog): match Gesdén ``Tratamientos`` against the Dentia
...
- feat(payment): negative ``PagoCli`` rows now create
  ``payments.Refund`` rows tied to a Dentia ``Payment`` instead
...
- fix(applied_treatment): hide migrated treatments from the
  ...
  imported plans.
- fix(professional): map canonical role ``doctor`` → Dentia
...
- fix(applied_treatment): migrated plans land in ``active`` instead of
  ...
  leaving them in Dentia's pre-confirmation state forced the
...
- feat(applied_treatment): map Gesdén ``IdTipoOdg`` to Dentia's
...
- fix(payment): the historical Payment mapper was producing rows that
  ...
  ``cash``/``card``/``bank_transfer`` etc. land in Dentia instead
...
- feat(mappers): new ``PatientClientLinkMapper`` plus client→patient
  ...
  the *client* (payer), not the patient — Dentia's flat
...
- feat(catalog): reuse existing Dentia catalog items when the
...
- feat(mappers): new ``AppliedTreatmentPhaseMapper`` attaches DPMF
  ...
  item. Per-session amount is left at zero (Dentia sessions carry
...
- feat(mappers): new ``CatalogVariantMapper`` resolves
  ...
  ``treatment_catalog_item`` (pipe-through). Dentia's catalog
  doesn't model a per-tariff axis, so the variant's canonical UUID
  resolves to the same Dentia row as its parent; pricing
...
- feat(mappers): new ``AppointmentMapper`` materialises DPMF
  ...
  Dentia's 7-state status enum, and emits warnings under
...
- fix(lifecycle): default ``_staging_root()`` to
  ...
  ``/var/lib/dentia/migration-import``. The previous hardcoded path
