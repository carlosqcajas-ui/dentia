...
For now only ``kind=chair`` produces real Dentia rows: Gesdén's
...
fall through to :class:`RawEntity` for forward-compat — when Dentia
...
# Dentia's seeded palette so the migrated cabinets blend in with
    ...
    async def _handle_chair(
        ...
        *,
        ...
        await ctx.resolver.set(
            ...
            canonical_uuid=canonical_uuid,
            source_system=source_system,
            dentia_table="cabinets",
            dentia_id=cabinet.id,
        )
        return cabinet.id
