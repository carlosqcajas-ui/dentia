...
class CatalogItemMapper:
        ...
        if effective_target_id is not None and effective_action != "create":
            ...
            ctx.db.add(
                ImportWarning(
                    ...
                    code=code,
                    message=(
                        f"Catálogo Gesdén '{name}' enlazado a Dentia id={effective_target_id}."
                    ),
                )
            )
    ...
    async def _find_match(
        ...
        Dentia catalog item match, or ``None`` when no acceptable
        candidate exists.
        ...
        2. If ``category_filter`` is supplied (Gesdén ``IdTipoODG``
           resolved to a seeded Dentia category), restrict the
