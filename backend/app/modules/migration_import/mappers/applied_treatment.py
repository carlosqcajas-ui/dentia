...
Dentia this requires three rows in cooperation:
...
# real. ``active`` is the post-acceptance state in Dentia's plan
...
# map to a known Dentia treatment vocabulary entry. Visible in
# warnings so the operator can backfill the mapping.
_FALLBACK_CLINICAL_TYPE = "migrated"

# Gesdén ``IdTipoOdg`` → Dentia ``TreatmentType`` resolution lives
...
# Once a treatment is realised, its Dentia ``clinical_type`` often
...
class AppliedTreatmentMapper:
    ...
    async def apply(
        ...
        *,
        ...
        if twin_uuid is not None:
            ...
            if twin_item_id is not None and twin_treatment_id is not None:
                await ctx.resolver.set(
                    ...
                    dentia_table="planned_treatment_items",
                    dentia_id=twin_item_id,
                )
                await ctx.resolver.set(
                    entity_type="applied_treatment_record",
                    canonical_uuid=canonical_uuid,
                    source_system=source_system,
                    dentia_table="treatments",
                    dentia_id=twin_treatment_id,
                )
            else:
        ...
        #   Dentia ``clinical_type``. These are the free-text rows
        ...
        if not was_resolved and id_tipo_odg not in (None, 0):
            await _warn(
                ...
                f"IdTipoOdg={id_tipo_odg} sin equivalente clínico en Dentia; "
        ...
        # use FecIni as an approximation so Dentia's reports have
        # a date to anchor on.
    ...
    async def _tooth_record_id(
        ...
        map; subsequent clinical activity in Dentia overlays state
        on top.
        """
        key = (ctx.clinic_id, patient_id, tooth_number)
