...
> Read-only diagnosis of Dentia `main` @ `e19e081`. Nothing here was applied to the
...
- **Clinic-configured From address is dead code** `email/service.py:188` — always stamps
  `noreply@dentia.com` → SPF/DMARC misalignment.
...
- DELETE returns 200+body in 4 routers (convention 204); HTTPException raised inside 22
  service methods (breaks agent-tool wrapping); `DENTIA_DEV_MODULE_SCAN` defaults True in
