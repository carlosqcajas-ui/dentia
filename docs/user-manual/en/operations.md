...
- Cloned Dentia repo (or equivalent deploy artefacts).
...
JSON variant: `./bin/dentia modules list --json`.

### Inspecting one module

```bash
./bin/dentia modules info billing
```
...
Bundled with every Dentia release. They auto-install on the first
boot of a fresh database. Reinstall if they ended up in `uninstalled`:

```bash
./bin/dentia modules install billing
...
docker compose exec backend pip install dentia-my-module
...
```bash
./bin/dentia modules upgrade my_module
./bin/dentia modules restart
```
...
```bash
./bin/dentia modules uninstall my_module
docker compose restart backend
```
...
| Channel | Command |
|---------|---------|
| CLI hint | `./bin/dentia modules restart` prints next step |
...
1. Inspect `frontend/modules.json` — it should list the module's layer
   path.
2. Run `./bin/dentia modules sync-frontend` to regenerate the file.
...
`dentia modules doctor` lists the module with its error. Options:

1. Fix the root cause (usually a migration or seed bug), then
   `dentia modules install <name>` + restart to retry.
2. Give up: `dentia modules orphan <name>` (marks uninstalled
...
- Restore the package (`pip install dentia-ghost`).
- Mark uninstalled:

  ```bash
  ./bin/dentia modules orphan ghost
  ```
...
| Community module page 404 | `modules.json` missing the layer path | `./bin/dentia modules sync-frontend` + frontend rebuild |
...
GitHub: https://github.com/dentia/dentia/issues — include the
output of:

```bash
./bin/dentia modules doctor --json
./bin/dentia modules info <affected-module> --json
